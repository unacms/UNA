...
class BxMessengerServices extends BxDol
{
    ...
    public function serviceGetSendForm($sParams = '')
    {
        ...
        if ($oForm->isSubmittedAndValid()){
           ...
           /* if(($iReply = bx_get('reply')) !== false)
            ...
            // !!###Agents Message — socket broadcast lives in sendMessage()
    ...
    /**
     * Active messenger agents (trigger=message) as profile units.
     * Returns [] when Studio Agents are absent or the AI runtime is missing.
     */
    public function serviceGetAgents($sParams = '')
    {
        // !!###Agents Message
        if (!$this->_iProfileId)
            return [];

        if (!class_exists('BxDolAi'))
            return [];

        $oAi = BxDolAi::getInstance();
        if (!$oAi || !method_exists($oAi, 'getAgentsByTriggerType'))
            return [];

        $aOptions = is_array($sParams) ? $sParams : (json_decode((string)$sParams, true) ?: []);
        $sTerm = isset($aOptions['term']) ? mb_strtolower(trim((string)$aOptions['term'])) : '';

        $aAgents = $oAi->getAgentsByTriggerType('message');
        if (empty($aAgents) || !is_array($aAgents))
            return [];

        $aResult = [];
        $aSeen = [];
        foreach ($aAgents as $aAgent) {
            $iProfileId = (int)($aAgent['profile_id'] ?? 0);
            if ($iProfileId <= 0 || isset($aSeen[$iProfileId]))
                continue;
            ...
            $sTitle = trim((string)($aAgent['title'] ?? ''));
            if ($sTitle === '')
                $sTitle = trim((string)($aAgent['name'] ?? ''));

            if ($sTerm !== '') {
                $sHay = mb_strtolower($sTitle . ' ' . $oProfile->getDisplayName());
                if (mb_strpos($sHay, $sTerm) === false)
                    continue;
            }

            $aProfile = BxDolProfile::getData($oProfile);
            if (empty($aProfile) || empty($aProfile['id']))
                continue;

            if ($sTitle !== '') {
                $sProfileName = trim((string)($aProfile['display_name'] ?? ''));
                $aProfile['title'] = $sTitle;
                $aProfile['display_name'] = $sProfileName !== '' && strcasecmp($sProfileName, $sTitle) !== 0
                    ? $sProfileName . ' (' . $sTitle . ')'
                    : $sTitle;
            }
            $aProfile['agent_id'] = (int)$aAgent['id'];

            $aSeen[$iProfileId] = true;
            $aResult[] = $aProfile;
        }
